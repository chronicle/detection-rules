...
def iso8601_datetime_utc(utc_date_time: str) -> datetime.datetime:
  ...
  try:
    ...
    # No microseconds? No problem, try to parse without them. If there's a
    # different parsing problem, it will surface below too.
    pass

  return datetime.datetime.strptime(
      f"{utc_date_time}+0000", "%Y-%m-%dT%H:%M:%SZ%z"
  )


def strftime(utc_date_time: datetime.datetime) -> str | None:
