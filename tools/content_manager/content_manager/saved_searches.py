...
class SavedSearches:
  ...
  @classmethod
  def parse_saved_search(cls, saved_search: Mapping[str, Any]) -> SavedSearch:
  ...
  def parse_saved_searches(
      cls, saved_searches: Sequence[Mapping[str, Any]]
  ) -> list[SavedSearch]:
    """Parse a list of saved searches into a list of SavedSearch objects."""
    parsed_saved_searches = []

    for saved_search in saved_searches:
      parsed_saved_searches.append(SavedSearches.parse_saved_search(saved_search))
  ...
  @classmethod
  def check_saved_search_config(cls, config: Mapping[str, Any]):
  ...
  def dump_saved_search_config(self):
    ...
    for saved_search in self.saved_searches:
      ...
      # a file. Its primary purpose is to force the YAML dumper to use the
      if "\n" in saved_search_config[saved_search.name]["query"]:
        saved_search_config[saved_search.name]["query"] = LiteralScalarString(
            saved_search.query
        )
  ...
  def get_remote_saved_searches(
    ...
    LOGGER.info("Retrieved a total of %s saved searches", raw_saved_searches_count)
  ...
  def update_remote_saved_searches(
    ...
    for local_saved_search in local_saved_searches.saved_searches:
      ...
      if not saved_search_resource_name:
        ...
        remote_saved_search = remote_saved_searches_dict[saved_search_resource_name]
        ...
        if local_saved_search.placeholder_names != remote_saved_search.placeholder_names:
          LOGGER.info(
        ...
        if local_saved_search.placeholder_descriptions != remote_saved_search.placeholder_descriptions:
          LOGGER.info(
            "Saved search %s - Placeholder descriptions are different in local "
            "and remote saved search. Remote saved search will be updated",
            saved_search_name
          )
          update_remote_saved_search = True
