...
def test_load_data_table_config():
  ...
  with pytest.raises(
      ...
      match=(
          r"Data table file not found with name .*\.csv in .*"
      ),
  ):
    DataTables.load_data_table_config(
        data_table_config_file=TEST_DATA_DIR / "test_data_table_config_missing_data_table_file.yaml",
...
def test_compare_data_table_contents():
  ...
  result = DataTables.compare_data_table_content(data_table_1_row_values=data_table_1_row_values, data_table_2_row_values=data_table_2_row_values)
...
def test_check_data_table_config():
  ...
  with pytest.raises(ruamel.yaml.constructor.DuplicateKeyError):
    DataTables.load_data_table_config(
        data_table_config_file=TEST_DATA_DIR / "test_data_table_config_duplicate_keys.yaml",
        data_tables_dir=TEST_DATA_TABLES_DIR
    )

  data_table_config = DataTables.load_data_table_config(
      data_table_config_file=TEST_DATA_TABLES_CONFIG_FILE, data_tables_dir=TEST_DATA_TABLES_DIR
  )
  ...
  # invalid key.
  with pytest.raises(DataTableConfigError, match=r"Invalid keys .* found for data table - "):
