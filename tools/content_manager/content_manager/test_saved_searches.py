...
def raw_test_saved_searches_fixture() -> list[dict[str, Any]]:
...
def test_parse_saved_searches(
    raw_test_saved_searches: list[dict[str, Any]],
):
  ...
  with pytest.raises(
    ...
    SavedSearch(
        ...
        query=(
            ...
            ' /Windows/ AND metadata.event_type = "USER_LOGIN" AND'
            ' security_result.action = "BLOCK" AND principal.hostname !='
...
def test_check_saved_search_config():
  ...
  with pytest.raises(ruamel.yaml.constructor.DuplicateKeyError):
    SavedSearches.load_saved_search_config(
        saved_search_config_file=TEST_DATA_DIR
        / "test_saved_search_config_duplicate_keys.yaml"
    )
  ...
  # invalid key
  saved_search_config["Blocked Windows Logins by Host"][
      "invalid_key"
  ] = "invalid"
  with pytest.raises(
