...
API reference:
https://cloud.google.com/chronicle/docs/reference/rest/v1alpha/projects.locations.instances.dataTables/delete
"""
...
def delete_data_table(
  ...
  Args:
    ...
    resource_name: The resource name of the data table to delete. Format:
      projects/{project}/locations/{location}/instances/{instance}/dataTables/{data_table_id}
    force (optional): If set to True, the data table will be deleted as well as
      all of its rows. If set to False and the data table contains one or more
      rows, the request will fail.
  ...
  for _ in range(max(max_retries, 0) + 1):
    response = http_session.request(method="DELETE", url=url, params=params)
