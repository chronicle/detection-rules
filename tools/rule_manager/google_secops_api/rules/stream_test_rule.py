            ...
            for result in parse_stream(response):
                if "detection" in result:
                    ...
                    if error.get("category") == "RULES_EXECUTION_ERROR":
                        ...
                        disconnection_reason = (
                            f"Connection aborted with error={error_dump}"
                        )
                        break
...
def test_rule(
    ...
    Args:
      ...
      start_time (optional): Start time of the time range of logs to test the
        rule over. If unspecified, will default to 12 hours before end_time.
      ...
      end_time (optional): End time of the time range of logs to test the rule
        over. Optional. The end time of the time range of events to test the
        rule text over. If unspecified, will either default to 12 hours after
      ...
      max_detections (optional): Maximum number of detections to return. The
        maximum number of detections to return. The service may return fewer
        than this value. If unspecified, at most 1,000 detections will be
