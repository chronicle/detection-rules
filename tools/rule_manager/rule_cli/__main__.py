...
import datetime  # Keep datetime for helper
import json
import logging
import os
import pathlib
...
def initialize_http_session() -> google.auth.transport.requests.AuthorizedSession:
    ...
    return auth.initialize_http_session(
        scopes=json.loads(os.environ["AUTHORIZATION_SCOPES"]).get("GOOGLE_SECOPS_API")
    )
...
class RuleOperations:
    ...
    def verify(cls, rule_file_path: pathlib.Path):
        """Make an API call to verify that a detection rule is a valid YARA-L rule."""
        http_session = initialize_http_session()
    ...
    @classmethod
    def verify_all(cls):
        """Verify that all detection rules are valid YARA-L rules using Google SecOps's API."""
        http_session = initialize_http_session()
        ...
        LOGGER.info(
            "Rule verification succeeded for %s rules",
            len(verify_rule_successes),
        )
    ...
    def stream_test(
        ...
        #       ROOT_DIR / f"rule_test_results_{rule_file_path.stem}_{current_time}.json"
        #   )
...
class ReferenceListOperations:
    ...
    def get(cls):
        ...
        if (
            len(remote_ref_lists.ref_lists) == 0
...
class RuleExclusionOperations:
    ...
    def get(cls):
        ...
        if (
            len(remote_rule_exclusions.rule_exclusions) == 0
        ):  # pylint: disable="g-explicit-length-test"
...
@click.group()
def cli():
    """Content Manager - Manage content such as rules, reference lists, and exclusions via the Google SecOps API."""
...
@rules.command(
    "update",
    short_help="Update rules in Google SecOps based on local rule files and config.",
)
def update_rules():
    """Update rules in Google SecOps based on local rule files and config."""
    LOGGER.info("Attempting to update rules in Google SecOps based on local rule files")
    RuleOperations.update()


@rules.command("verify", short_help="Verify a single rule file is a valid YARA-L rule.")
@click.option(
    "--rule-file-path",
    "-f",
    required=True,
...
@reference_lists.command(
    "get",
    short_help="Retrieve the latest reference lists from Google SecOps and update local files.",
)
...
@reference_lists.command(
    "update",
    short_help="Update reference lists in Google SecOps based on local files and config.",
)
...
@rule_exclusions.command(
    "get",
    short_help="Retrieve the latest rule exclusions from Google SecOps and update local config.",
)
