...
from google_secops_api.reference_lists.update_reference_list import (
...
class ReferenceLists:
        ...
        for ref_list in ref_lists:
            ...
            try:
                parsed_ref_list = ReferenceList(
                    ...
                    rule_associations_count=ref_list.get(
    ...
    def load_ref_lists(
        ...
        for ref_list_file_path in ref_list_files:
            ...
            ref_list = ReferenceList(
                name=ref_list_name,
                resource_name=ref_list_config[ref_list_name].get(
                    "resource_name"
                ),
    ...
    def dump_ref_lists(self, ref_lists_dir: pathlib.Path = REF_LISTS_DIR):
        ...
        for ref_list in self.ref_lists:
            ...
            with open(
                ref_list_file_path, "w", encoding="utf-8"
            ) as ref_list_file:
                for entry in ref_list.entries:
    ...
    def get_remote_ref_lists(
        ...
        LOGGER.info(
            "Attempting to retrieve all reference lists from Google SecOps"
        )
        while True:
            ...
            if retrieved_ref_lists is not None:
                LOGGER.info(
                    "Retrieved %s reference lists", len(retrieved_ref_lists)
                )
        ...
        LOGGER.info(
            "Retrieved a total of %s reference lists", raw_ref_lists_count
        )

        parsed_ref_lists = ReferenceLists.parse_ref_lists(
            ref_lists=raw_ref_lists
        )
    ...
    def update_remote_ref_lists(
        ...
        if (
            len(local_ref_lists.ref_lists) == 0
        ):  # pylint: disable="g-explicit-length-test"
