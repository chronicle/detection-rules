...
RULE_EXCLUSION_TYPES = Literal[
    "DETECTION_EXCLUSION"
]  # pylint: disable="invalid-name"
EXCLUSION_APPLICATIONS = Literal[
    "curated_rule_sets", "curated_rules"
]  # pylint: disable="invalid-name"
...
class RuleExclusion(pydantic.BaseModel):
    ...
    exclusion_applications: Mapping[
        EXCLUSION_APPLICATIONS, Sequence[str] | None
    ]
...
class RuleExclusions:
    ...
    def parse_rule_exclusion(
        ...
        if rule_exclusion["deployment_state"].get(
            ...
            try:
                rule_exclusions_parsed.append(
                    RuleExclusion(
                        ...
                        resource_name=rule_exclusion_config_entry.get(
                            "resource_name"
                        ),
                        type=rule_exclusion_config_entry["type"],
                        create_time=rule_exclusion_config_entry.get(
                            "create_time"
                        ),
                        update_time=rule_exclusion_config_entry.get(
                            "update_time"
                        ),
    ...
    def get_remote_rule_exclusions(
        ...
        LOGGER.info(
            "Attempting to retrieve all rule exclusions from Google SecOps"
        )
        while True:
            (
        ...
        while True:
            ...
            if retrieved_exclusion_deployments is not None:
                ...
                rule_exclusion_deployments.extend(
                    retrieved_exclusion_deployments
                )
    ...
    def update_remote_rule_exclusions(
        ...
        if (
            ...
            for remote_rule_exclusion in remote_rule_exclusions.rule_exclusions:
                remote_rule_exclusions_dict[
                    remote_rule_exclusion.resource_name
        ...
        for local_rule_exclusion in local_rule_exclusions.rule_exclusions:
            ...
            if not rule_exclusion_resource_name:
                ...
                LOGGER.info(
                    "Created new rule exclusion %s", remote_rule_exclusion.name
                )
            ...
            if (
                ...
                update_summary[
                    "detection_exclusion_applications_updated"
                ].append((rule_exclusion_name, rule_exclusion_resource_name))
