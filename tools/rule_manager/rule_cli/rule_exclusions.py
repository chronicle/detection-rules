...
from google_secops_api.findings_refinements.update_findings_refinement_deployment import (
...
class RuleExclusions:
  ...
  def parse_rule_exclusion(
    ...
    if rule_exclusion["deployment_state"].get("enabled") is True:
      pass
    else:
      rule_exclusion["deployment_state"]["enabled"] = False

    if rule_exclusion["deployment_state"].get("archived") is True:
      pass
    else:
    ...
    if rule_exclusion["deployment_state"].get("detectionExclusionApplication"):
      exclusion_applications = {
          "curated_rule_sets": rule_exclusion["deployment_state"][
              "detectionExclusionApplication"
          ].get("curatedRuleSets"),
          "curated_rules": rule_exclusion["deployment_state"][
              "detectionExclusionApplication"
          ].get("curatedRules"),
      }
    else:
  ...
  def load_rule_exclusion_config(
    ...
    LOGGER.info(
        "Loading rule exclusion config file from %s", rule_exclusion_config_file
    )
  ...
  def dump_rule_exclusion_config(self):
    ...
    with open(
      ...
      yaml.dump(
          rule_exclusion_config, rule_exclusion_config_file, sort_keys=True
      )
  ...
  def get_remote_rule_exclusions(
    ...
    while True:
      ...
      if retrieved_rule_exclusions is not None:
        LOGGER.info(
            "Retrieved %s rule exclusions", len(retrieved_rule_exclusions)
        )
    ...
    LOGGER.info(
        "Attempting to retrieve deployment state for %s rule exclusions",
        raw_rule_exclusions_count,
    )
    for rule_exclusion in raw_rule_exclusions:
      rule_exclusion["deployment_state"] = get_findings_refinement_deployment(
          http_session=http_session, resource_name=rule_exclusion["name"]
      )
  ...
  def update_remote_rule_exclusion_state(
    ...
    if (
      ...
      update_refinement_findings_deployment(
          ...
          updates={
              "detection_exclusion_application": local_rule_exclusion.exclusion_applications
          },
      )
      rule_exclusion_updates["detection_exclusion_applications_updated"] = True

    LOGGER.debug(
        "%s - Checking if the rule exclusion should be archived", log_msg_prefix
    )
